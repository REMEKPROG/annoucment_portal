...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if($_FILES["image-uploaded"]["error"] === 0 && $_FILES["image-uploaded"]["size"] <= 5242880) {
        ...
        if($moveImage) {
            $imgToDB = $dir;
            $scale = scaleImage($dir, $dir, 500, 500);
        } else {
    ...
    try {
    $addAnnoucmentSQL = "INSERT INTO serwis_ogloszeniowy.annoucments(id_user, title, description, contents, img)
    VALUES(:id_user, :title, :description, :contents, :img)";
