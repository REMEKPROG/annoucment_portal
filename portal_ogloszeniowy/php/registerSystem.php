...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $komunikat = "";
    ValidateForm();
    if (isset($_POST["name"]) && (mb_strlen($_POST["name"])) <= 50 && preg_match('/^[A-ZŻŹĆĄŚĘŁÓŃ][a-zżźćńółęąś]{1,29}(-[A-ZŻŹĆĄŚĘŁÓŃ][a-zżźćńółęąś]{1,29})?$/u', $_POST["name"])) {
        $name = trim($_POST["name"]);
    } else {
        $komunikat .= "imie jest za długie lub niepoprawne";
        $flag = false;
    }
    if (isset($_POST["surname"]) && (mb_strlen($_POST["surname"])) <= 50 && preg_match('/^[A-ZŻŹĆĄŚĘŁÓŃ][a-zżźćńółęąś]{1,39}(-[A-ZŻŹĆĄŚĘŁÓŃ][a-zżźćńółęąś]{1,39})?$/u', $_POST["surname"])) {
        $surname = trim($_POST["surname"]);
    } else {
        $komunikat .= "nazwisko jest za długie lub niepoprawne";
        $flag = false;
    }
...
<body>
    <main class="register-main">
        ...
        <h2 class="alerth2"><?php echo $komunikat ?></h2>
    </main>
</body>
</html>